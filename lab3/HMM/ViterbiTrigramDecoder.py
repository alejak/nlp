...
class ViterbiTrigramDecoder(object):
    ...
    def viterbi(self, s):
        ...
        # Initialization
        # 0th time step points to START_END 
        self.backptr[0, :, :] = Key.START_END
        # 0th time step initialised with transition probability of states from
        # START_END and observation likelihoods of first character in string
        self.v[0, Key.START_END, :] = self.a[Key.START_END, Key.START_END, :] + self.b[index[0], :]

        # Induction step
        for t in range(1, len(s)):  # For each time step
            t_1 = list(Key.neighbour[index[t-1]])
            t_1.append(Key.index_to_char(index[t-1]))
            t_2 = list(Key.neighbour[index[t-2]])
            t_2.append(Key.index_to_char(index[t-2]))

            for k in range(Key.NUMBER_OF_CHARS):  # For each state in column
                for j in t_1:  # For each state in column t-1
                    j = Key.char_to_index(j)
                    max = -float("inf")
                    iMax = 0
                    for i in t_2:  # For each state in column t-2
                        i = Key.char_to_index(i)
                        stateProb = self.v[t-1, i, j] + self.a[i, j, k] + self.b[index[t], k]
                        if stateProb > max:  # Find the maximum stateProb value and index for it
                            max = stateProb
                            iMax = i

                    self.v[t, j, k] = max
                    self.backptr[t, j, k] = iMax

        # Set best endpoint and backtrace from there
        bestPathPointer = Key.START_END
        bPP_1 = Key.START_END
        bestPath = ''
        t = len(s)-1

        while t > 1:
            bestPathPointer, bPP_1 = self.backptr[t, bestPathPointer, bPP_1], bestPathPointer
            bestPath += Key.index_to_char(bestPathPointer)
            t -= 1
