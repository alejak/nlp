...
class ViterbiBigramDecoder(object):
    ...
    def init_a(self, filename):
        ...
        with codecs.open(filename, 'r', 'utf-8') as f:
            for line in f:
                i, j, d = [func(x) for func, x in zip(
                    [int, int, float], line.strip().split(' '))]
                self.a[i][j] = d
    ...
    def viterbi(self, s):
        ...
        if s == "":
            ...
            for t in range(1,len(s)):
                for st in range(Key.NUMBER_OF_CHARS):
                    self.v[t,st] = max(self.v[t-1,:] + self.a[:,st]) + self.b[index[t],st]
                    self.backptr[t,st] = np.argmax(self.v[t-1,:] + self.a[:,st]+ self.b[index[t],st])

        bestPathPointer = np.argmax(self.v[len(s)-1,:])
        bestPath = ''
        t = len(s)-1
        while t > 0:
            bestPathPointer=self.backptr[t,bestPathPointer]
...
def main():
    ...
    parser.add_argument('--probs', '-p', type=str,
                        required=True, help='bigram probabilities file')
    ...
    if arguments.check:
        ...
        if response_data['correct']:
            ...
            print('Your results:')
            print(result)
            print('Your answer is {0:.0f}% similar to that of the server'.format(
